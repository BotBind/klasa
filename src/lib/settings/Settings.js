...
class Settings {
  ...
  async reset(keys, guild, { avoidUnconfigurable = false, force = false } = {}) {
    ...
    if (Array.isArray(keys)) {
      ...
      for (const key of keys) {
        ...
        if (!path) {
          result.errors.push(guild && guild.language ? guild.language.get('COMMAND_CONF_GET_NOEXT', key) : `The path ${key} does not exist in the current schema, or does not correspond to a piece.`);
          continue;
        }
  ...
  list(message, path) {
    ...
    if (keysTypes.length) {
      for (const keyType of keysTypes.sort()) {
        array.push(`= ${toTitleCase(keyType)}s =`, ...keys[keyType].sort().map(key => `${key.padEnd(longest)} :: ${this.resolveString(message, folder.get(key))}`), '');
      }
    }
    return array.join('\n');
  }
  ...
  async _update(entries, guild, options) {
    ...
    for (const [key, value] of entries) {
      ...
      if (!path) {
        result.errors.push(guild ? guild.language.get('COMMAND_CONF_GET_NOEXT', key) : `The path ${key} does not exist in the current schema, or does not correspond to a piece.`);
        continue;
      }
      if (!path.piece.array && Array.isArray(value)) {
        result.errors.push(guild ? guild.language.get('SETTING_GATEWAY_KEY_NOT_ARRAY', key) : `The path ${key} does not store multiple values.`);
        continue;
      }
    ...
    const parsed =
      value === null ?
        ...
        await (Array.isArray(value) ?
          ...
          piece.parse(value, guild).catch(error => {
